...
class AuthProvider extends ChangeNotifier {
  ...
  // ✅ NEW: Expose ApiClient for partner onboarding
  ...
  /// ✅ NEW: Refresh account data from backend (used after partner onboarding)
  Future<void> refreshAccount() async {
    ...
    try {
      ...
      if (response.success && response.data != null) {
        ...
        if (_account!.roles.isNotEmpty) {
          // Keep current role if still valid, otherwise switch to first available
          if (!_account!.roles.contains(_activeRole)) {
            _activeRole = _account!.roles.first;
            await _secureStorage.setLastActiveRole(_activeRole.name.toUpperCase());
          }
        }
      ...
      throw Exception('Failed to refresh account: $e');
    }
  }
