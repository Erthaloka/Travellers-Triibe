...
/// CRITICAL: All values are READ-ONLY, calculated server-side
library;
...
/// Payment preview page - READ-ONLY bill breakdown
...
class _PaymentPreviewPageState extends State<PaymentPreviewPage> {
  ...
  Future<void> _processPayment() async {
    ...
    if (result.success && result.data != null) {
      ...
      // For now, navigate to success page with order data
      // In production, open Razorpay checkout and verify payment

      // Simulate Razorpay payment (in production, use razorpay_flutter package)
      await _simulateRazorpayPayment(paymentData);
    } else {
      setState(() {
        _isProcessing = false;
  ...
  Future<void> _simulateRazorpayPayment(PaymentData paymentData) async {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Discount
          _buildAmountRow(
            label: 'Discount (${widget.data.discountPercent.toStringAsFixed(0)}%)',
