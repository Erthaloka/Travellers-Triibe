...
class BillsService {
  ...
  /// User: Verify Razorpay payment with backend
  Future<VerifyPaymentResult> verifyPayment({
    ...
    if (response.success && response.data != null) {
      try {
        final data = response.data!['data'] ?? response.data!;
        return VerifyPaymentResult.success(VerifiedPayment.fromJson(data));
      } catch (e) {
        return VerifyPaymentResult.failure('Invalid response format');
      }
    }

    return VerifyPaymentResult.failure(
      response.error?.message ?? 'Payment verification failed',
    );
  }
}
...
class VerifiedPayment {
  ...
  factory VerifiedPayment.fromJson(Map<String, dynamic> json) {
    return VerifiedPayment(
      ...
      paymentId: json['paymentId'] ?? json['razorpayPaymentId'] ?? '',
      status: json['status'] ?? json['order']?['status'] ?? 'COMPLETED',
      amount: (json['amount'] ?? json['order']?['finalAmount'] as num?)?.toDouble() ?? 0.0,
      merchantName: json['merchantName'] ?? json['merchant']?['businessName'] ?? '',
    );
  }
}
...
class VerifyPaymentResult {
  final bool success;
  final VerifiedPayment? data;
  final String? errorMessage;

  VerifyPaymentResult._({required this.success, this.data, this.errorMessage});

  factory VerifyPaymentResult.success(VerifiedPayment data) =>
      VerifyPaymentResult._(success: true, data: data);

  factory VerifyPaymentResult.failure(String message) =>
      VerifyPaymentResult._(success: false, errorMessage: message);
}